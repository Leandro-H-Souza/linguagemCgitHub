...
void SalvarDados(struct aluno Turma[], int totalDeAlunos) {
    ...
    for (int i = 0; i < totalDeAlunos; i++) {
        fprintf(arquivo, "%s, %d, %c, %.2f, %.2f, %.2f\n",
                Turma[i].nomeAluno,
                Turma[i].idadeAluno,
                Turma[i].sexo,
                Turma[i].nota1,
    ...
    char comando[100];
    sprintf(comando, "notepad %s", Registro_Arquivo);
    system(comando);
}
...
void CarregandoDados(struct aluno Turma[], int *totalDeAlunos) {
    ...
    for (int i = 0; i < *totalDeAlunos; i++) {
        if (fscanf(arquivo, " %79[^,], %d, %c, %f, %f, %f\n",
               Turma[i].nomeAluno,
               &Turma[i].idadeAluno,
               &Turma[i].sexo,
               &Turma[i].nota1,
...
void addAluno(struct aluno Turma[], int *totalDeAlunos) {
    ...
    do {
        ...
        scanf(" %c", &Turma[*totalDeAlunos].sexo);
    } while (Turma[*totalDeAlunos].sexo != 'm' && Turma[*totalDeAlunos].sexo != 'f');
...
void calculoMedia(struct aluno Turma[], int totalDeAlunos) {
    ...
    for (int i = 0; i < totalDeAlunos; i++) {
        if (strcmp(Turma[i].nomeAluno, buscarnome) == 0) {
            ...
            printf("Média do aluno %s: %.2f\n", Turma[i].nomeAluno, Turma[i].media);
            alunoEncontrado = 1;
            break;
        }
    }
